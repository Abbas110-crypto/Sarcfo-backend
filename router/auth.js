...
require("../db/connect");
const {  ContactPageUser, AdminSignup_PageUser,Blog_Database } = require("../db/userSchema");
...
router.get('/admin/dashboard/contact', async (req, res) => {
  try {
...
router.post('/blogs', uploadOriginal.single('image'), async (req, res) => {
  try {
    const { title, content, description,DateButton } = req.body;
    ...
    console.log('Received data:', { title, content, description ,DateButton, image: req.file });
...
router.get('/blogs/top3', async (req, res) => {
  try {
    ...
    console.log(convertedData, "blog converting");
